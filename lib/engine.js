...
module.exports = function(thorin, engine, opt) {
  ...
  (() => {
    ...
    switch(engine.type) {
      case "nunjucks":
        ...
        let tmpEnv = instance.configure(path.normalize(thorin.root + '/../'), envOpt);
  ...
  * */
  plugin.render = function DoRender(templateName, locals, onDone, _includeGlobals) {
    ...
    try {
      ...
      if(locals instanceof thorin.Intent) {
        ...
        if(_includeGlobals === false) {
          localData = thorin.util.extend({
            asset: globalLocals.asset
          },locals);
        } else {
  ...
  * */
  ...
  /* Returns the global locals object */
  plugin.getLocals = () => globalLocals;
  /*
